...
JSON: {"recommended_orders":[{"order_id":"uuid","match_score":9.5,"explanation":"причина"}],"explanation":"общее"}`,
	...
	// Пробуем распарсить прямой JSON или JSON из markdown-блока.
	if err := json.Unmarshal([]byte(response), &result); err != nil {
		candidate := extractJSONObject(response)
		if candidate == "" || json.Unmarshal([]byte(candidate), &result) != nil {
	...
	for _, rec := range result.RecommendedOrders {
		score := normalizeMatchScore(rec.MatchScore)
	...
	// Если модель вернула шкалу/оценки ниже порога, но рекомендации есть — берем лучшие.
	if len(ordersWithScores) == 0 && len(result.RecommendedOrders) > 0 {
		for _, rec := range result.RecommendedOrders {
			id, err := uuid.Parse(rec.OrderID)
			if err != nil {
				continue
			}
			score := normalizeMatchScore(rec.MatchScore)
...
func extractJSONObject(raw string) string {
	...
	if strings.Contains(text, "```") {
		startFence := strings.Index(text, "```")
		if startFence >= 0 {
			rest := text[startFence+3:]
			if strings.HasPrefix(strings.TrimSpace(rest), "json") {
				jsonIdx := strings.Index(rest, "\n")
				if jsonIdx >= 0 {
					rest = rest[jsonIdx+1:]
				}
			}
			...
			if endFence > 0 {
				candidate := strings.TrimSpace(rest[:endFence])
...
func normalizeMatchScore(v float64) float64 {
	if v <= 0 {
		return 0
	}
	// 0..1 -> 0..10
...
JSON: {"recommended_amount":1000,"min_amount":800,"max_amount":1200,"recommended_days":14,"min_days":10,"max_days":20,"explanation":"причина"}`,
	...
	// Try to extract JSON from markdown fences or text
...
func (c *Client) FindSuitableFreelancers(
	...
	profilesByID := make(map[uuid.UUID]*models.Profile, len(freelancerProfiles))
	for _, profile := range freelancerProfiles {
		if profile == nil {
			continue
		}
...
JSON: {"recommended_freelancers":[{"user_id":"uuid","match_score":9.5,"explanation":"причина"}]}`,
	...
	if err := json.Unmarshal([]byte(response), &result); err != nil {
		...
		if rawRecs, ok := parsed["recommended_freelancers"].([]interface{}); ok {
			for _, raw := range rawRecs {
				...
				score := 0.0
				switch v := obj["match_score"].(type) {
				case float64:
					score = v
				case int:
					score = float64(v)
				}
				result.RecommendedFreelancers = append(result.RecommendedFreelancers, struct {
					UserID      string  `json:"user_id"`
					MatchScore  float64 `json:"match_score"`
					Explanation string  `json:"explanation"`
				}{
	...
	if len(result.RecommendedFreelancers) == 0 {
		suitable := make([]models.SuitableFreelancer, 0, 5)
		for i := 0; i < len(freelancerProfiles) && i < 5; i++ {
			displayName := freelancerProfiles[i].DisplayName
			...
			suitable = append(suitable, models.SuitableFreelancer{
				...
				MatchScore:  7.0,
				Explanation: "Потенциально релевантен по профилю и активным навыкам для этого заказа.",
			})
		}
		return suitable, nil
	}
	...
	for _, rec := range result.RecommendedFreelancers {
		if id, err := uuid.Parse(rec.UserID); err == nil {
			displayName := "Исполнитель"
			if profile, ok := profilesByID[id]; ok && strings.TrimSpace(profile.DisplayName) != "" {
				displayName = profile.DisplayName
			}
			explanation := strings.TrimSpace(rec.Explanation)
			if explanation == "" {
				explanation = "Подходит по стеку и опыту для этого заказа."
			}
			suitable = append(suitable, models.SuitableFreelancer{
				UserID:      id,
				DisplayName: displayName,
...
1. Название заказа (title) - короткое, профессиональное и человекочитаемое название (строка, 4-10 слов)
2. Описание заказа (description) - четкое ТЗ в 4-8 предложениях, готовое к публикации (строка)
3. Навыки (skills) - список технологий/инструментов, которые нужны для выполнения заказа (массив строк, минимум 2-3 навыка)
4. Бюджет (budget_min и budget_max) - минимальная и максимальная стоимость в рублях (числа)
5. Срок (deadline_days) - количество дней на выполнение от сегодня (число)
6. Файлы (needs_attachments) - нужны ли прикрепленные файлы (boolean)
7. Описание файлов (attachment_description) - зачем нужны файлы (строка, если needs_attachments = true)
...
{
  "title": "UX/UI дизайн Telegram Mini App для игровой платформы",
  "description": "Нужно разработать UX/UI дизайн Telegram Mini App для игровой платформы в стиле онлайн-казино. Требуется проработать пользовательские сценарии, архитектуру экранов, визуальную концепцию, состояния интерфейсов и адаптацию под мобильные устройства. Подготовить UI-kit с типографикой, цветами, кнопками, карточками и модальными окнами. Передать макеты в Figma с интерактивным прототипом ключевых пользовательских потоков.",
...
Определи оптимальный бюджет в рублях (минимальная и максимальная стоимость) и рекомендуемый срок выполнения в днях.
...
ВАЖНО: Ответь ТОЛЬКО валидным JSON без дополнительного текста:
{
  ...
  "deadline_days": 14
	...
	messages := []map[string]string{
		{"role": "system", "content": "Ты помощник для фриланс-платформы. Анализируй заказы и предлагай оптимальный бюджет и срок выполнения. Всегда отвечай валидным JSON без дополнительного текста."},
		{"role": "user", "content": prompt},
	}
