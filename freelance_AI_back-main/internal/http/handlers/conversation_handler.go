...
func (h *ConversationHandler) GetOrderChat(c *gin.Context) {
	...
	if err != nil {
		common.RespondInternalError(c, "не удалось получить профиль исполнителя")
		return
	}
	...
	if err != nil {
		common.RespondInternalError(c, "не удалось получить профиль собеседника")
		return
	}
...
func (h *ConversationHandler) GetConversation(c *gin.Context) {
	...
	participantID, err := common.ParseUUIDParam(c, "participantId")
	if err != nil {
		common.RespondBadRequest(c, "неверный идентификатор участника")
		return
	}
...
func (h *ConversationHandler) SendMessage(c *gin.Context) {
	...
	if content == "" && len(attachmentMediaIDs) == 0 {
		common.RespondBadRequest(c, "сообщение должно содержать текст или вложения")
		return
	}
	...
	if h.hub != nil {
		...
		if conversation.ClientID == userID {
			_ = h.hub.BroadcastToUser(conversation.FreelancerID, "chat.message.created", payload)
		} else {
			_ = h.hub.BroadcastToUser(conversation.ClientID, "chat.message.created", payload)
		}
	}
	...
	common.RespondJSON(c, http.StatusCreated, response)
}
...
func (h *ConversationHandler) UpdateMessage(c *gin.Context) {
	...
	messageID, err := common.ParseUUIDParam(c, "messageId")
	if err != nil {
		common.RespondBadRequest(c, "неверный идентификатор сообщения")
		return
	}
	...
	common.RespondJSON(c, http.StatusOK, updatedMessage)
}
...
func (h *ConversationHandler) DeleteMessage(c *gin.Context) {
	...
	common.RespondJSON(c, http.StatusOK, gin.H{"message": "сообщение успешно удалено"})
}
...
func (h *ConversationHandler) AddMessageReaction(c *gin.Context) {
	...
	common.RespondJSON(c, http.StatusCreated, gin.H{"reaction": reaction})
}
...
func (h *ConversationHandler) RemoveMessageReaction(c *gin.Context) {
	...
	common.RespondJSON(c, http.StatusOK, gin.H{"message": "реакция успешно удалена"})
}
...
func (h *ConversationHandler) ListMyConversations(c *gin.Context) {
	...
	for _, conv := range conversations {
		...
		if err == nil && lastMessage != nil {
			// Загружаем вложения для последнего сообщения
			attachments, attErr := h.orders.GetMessageAttachments(c.Request.Context(), lastMessage.ID)
		...
		unreadCount, err := h.orders.CountUnreadMessages(c.Request.Context(), conv.ID, userID)
		if err == nil {
...
func (h *ConversationHandler) StreamSummarizeConversation(c *gin.Context) {
	...
	if err != nil {
		writeSSEErrorEventAndFlush(c.Writer, flusher, err.Error())
	}
}
