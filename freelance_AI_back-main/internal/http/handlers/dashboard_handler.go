...
func (h *DashboardHandler) GetDashboardData(c *gin.Context) {
	...
	// Cache the result
	// Cache for 1 hour if no AI.
	// For AI responses we cache aggressively only when AI data exists.
	// This prevents a long-lived "empty AI" dashboard if AI temporarily fails.
	cacheTTL := 1 * time.Hour
	if includeAI {
		hasAIData := false
		if dashboardData.AIRecommendations != nil {
			if len(dashboardData.AIRecommendations.RecommendedOrders) > 0 ||
				len(dashboardData.AIRecommendations.SuitableFreelancers) > 0 ||
				dashboardData.AIRecommendations.Explanation != "" {
				hasAIData = true
			}
		}
		if hasAIData {
	...
	if userRole == "freelancer" {
		...
		if recommendedOrders == nil {
			if h.orderService != nil {
				...
				// Не держим пустые рекомендации сутки: для пустого результата кэш короткий.
				cacheTTL := 24 * time.Hour
