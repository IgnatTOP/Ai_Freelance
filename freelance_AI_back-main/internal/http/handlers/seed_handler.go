...
func (h *SeedHandler) SeedRealistic(c *gin.Context) {
	...
	reset := c.DefaultQuery("reset", "1") != "0"
	numUsersStr := c.DefaultQuery("num_users", "40")
	numOrdersStr := c.DefaultQuery("num_orders", "120")
	numUsers, _ := strconv.Atoi(numUsersStr)
	numOrders, _ := strconv.Atoi(numOrdersStr)
	if numUsers < 1 {
		numUsers = 40
	}
	if numOrders < 1 {
	...
	result, err := h.extendedSeedService.SeedRealisticDataWithOptions(c.Request.Context(), service.ExtendedSeedOptions{
		Reset:     reset,
		NumUsers:  numUsers,
		NumOrders: numOrders,
	})
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"portfolio_created":     result.PortfolioCreated,
		"notifications_created": result.NotificationsCreated,
		"escrow_created":        result.EscrowCreated,
		"withdrawals_created":   result.WithdrawalsCreated,
		"disputes_created":      result.DisputesCreated,
		"reports_created":       result.ReportsCreated,
	})
}
