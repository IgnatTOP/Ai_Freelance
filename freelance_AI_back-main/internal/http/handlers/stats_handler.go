...
// GetPublicStats возвращает агрегированную статистику платформы для публичного лендинга.
func (h *StatsHandler) GetPublicStats(c *gin.Context) {
	freelancersTotal, err := h.users.CountFreelancers(c.Request.Context())
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "не удалось получить количество фрилансеров"})
		return
	}

	completedProjects, err := h.orders.CountCompletedOrders(c.Request.Context())
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "не удалось получить количество завершённых проектов"})
		return
	}

	marketplaceVolume, err := h.orders.SumCompletedOrderVolume(c.Request.Context())
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "не удалось получить оборот платформы"})
		return
	}

	averageRating, err := h.orders.GetMarketplaceAverageRating(c.Request.Context())
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "не удалось получить средний рейтинг"})
		return
	}

	c.JSON(http.StatusOK, gin.H{
		"freelancers_total":  freelancersTotal,
		"completed_projects": completedProjects,
		"marketplace_volume": math.Round(marketplaceVolume),
