	...
	return func(c *gin.Context) {
		...
		if len(c.Errors) > 0 {
			...
			c.JSON(statusCode, gin.H{
				"success": false,
				"error": gin.H{
					"code":    legacyStatusToCode(statusCode),
					"message": message,
				},
			})
		}
...
func legacyStatusToCode(status int) string {
	switch status {
	case http.StatusBadRequest:
		return string(apperror.ErrCodeBadRequest)
	case http.StatusUnauthorized:
		return string(apperror.ErrCodeUnauthorized)
	case http.StatusForbidden:
		return string(apperror.ErrCodeForbidden)
	case http.StatusNotFound:
		return string(apperror.ErrCodeNotFound)
	case http.StatusConflict:
		return string(apperror.ErrCodeConflict)
	default:
		return string(apperror.ErrCodeInternal)
	}
}
