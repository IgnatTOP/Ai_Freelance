...
func (r *ConversationRepositoryAdapter) Touch(ctx context.Context, id uuid.UUID) error {
	query := `UPDATE conversations SET updated_at = NOW() WHERE id = $1`
	_, err := r.db.ExecContext(ctx, query, id)
	if err != nil {
		return apperror.Wrap(err, apperror.ErrCodeDatabaseError, "не удалось обновить время беседы")
	}
	return nil
}
...
func (r *ConversationRepositoryAdapter) FindByID(ctx context.Context, id uuid.UUID) (*entity.Conversation, error) {
	...
	query := `
		SELECT
			id,
			COALESCE(order_id, '00000000-0000-0000-0000-000000000000'::uuid) AS order_id,
			client_id,
			freelancer_id,
			created_at,
			updated_at
...
func (r *MessageRepositoryAdapter) Create(ctx context.Context, msg *entity.Message) error {
	query := `
		INSERT INTO messages (id, conversation_id, author_type, author_id, content, created_at, updated_at)
		SELECT
			$1,
			$2,
			CASE
				WHEN c.client_id = $3 THEN 'client'::message_author
				WHEN c.freelancer_id = $3 THEN 'freelancer'::message_author
				ELSE 'system'::message_author
			END,
			$3,
			$4,
			$5,
		...
		WHERE c.id = $2
	`
	res, err := r.db.ExecContext(ctx, query, msg.ID, msg.ConversationID, msg.SenderID, msg.Content, msg.CreatedAt, msg.UpdatedAt)
	if err != nil {
		return apperror.Wrap(err, apperror.ErrCodeDatabaseError, "не удалось создать сообщение")
	}
	rows, err := res.RowsAffected()
	if err != nil {
		return apperror.Wrap(err, apperror.ErrCodeDatabaseError, "не удалось проверить создание сообщения")
	}
	if rows == 0 {
		return apperror.New(apperror.ErrCodeNotFound, "беседа не найдена")
	}
	return nil
}

func (r *MessageRepositoryAdapter) Update(ctx context.Context, msg *entity.Message) error {
	query := `UPDATE messages SET content = $2, updated_at = $3 WHERE id = $1`
	_, err := r.db.ExecContext(ctx, query, msg.ID, msg.Content, msg.UpdatedAt)
	if err != nil {
...
func (r *MessageRepositoryAdapter) FindByID(ctx context.Context, id uuid.UUID) (*entity.Message, error) {
	...
	query := `
		SELECT
			id,
			conversation_id,
			COALESCE(author_id, '00000000-0000-0000-0000-000000000000'::uuid) AS sender_id,
			content,
