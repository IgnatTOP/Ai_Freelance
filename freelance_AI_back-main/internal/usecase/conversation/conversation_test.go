...
func (m *mockConversationRepository) Touch(ctx context.Context, id uuid.UUID) error {
...
func TestGetOrderChatUseCase_CreatesConversation(t *testing.T) {
	convRepo := newMockConversationRepository()
	orderRepo := newMockOrderRepository()
	uc := conversation.NewGetOrderChatUseCase(convRepo, orderRepo)
	...
	conv, err := uc.Execute(context.Background(), order.ID, clientID)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if conv.ClientID != clientID || conv.FreelancerID != freelancerID {
		t.Fatalf("unexpected participants: client=%s freelancer=%s", conv.ClientID, conv.FreelancerID)
	}
}

func TestGetOrderChatUseCase_UsesExistingConversation(t *testing.T) {
	convRepo := newMockConversationRepository()
	orderRepo := newMockOrderRepository()
	uc := conversation.NewGetOrderChatUseCase(convRepo, orderRepo)
	...
	convRepo.conversations[existing.ID] = existing

	conv, err := uc.Execute(context.Background(), order.ID, freelancerID)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestGetOrderChatUseCase_NoFreelancer(t *testing.T) {
	convRepo := newMockConversationRepository()
	orderRepo := newMockOrderRepository()
	uc := conversation.NewGetOrderChatUseCase(convRepo, orderRepo)
	...
	_, err := uc.Execute(context.Background(), order.ID, clientID)
	if err == nil {
		t.Fatal("expected error when order has no freelancer")
	}
...
func TestGetOrderChatUseCase_Forbidden(t *testing.T) {
	convRepo := newMockConversationRepository()
	orderRepo := newMockOrderRepository()
	uc := conversation.NewGetOrderChatUseCase(convRepo, orderRepo)
	...
	_, err := uc.Execute(context.Background(), order.ID, otherUserID)
	if err == nil {
