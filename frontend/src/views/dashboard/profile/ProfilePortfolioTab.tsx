    ...
    return (
        <div className="mt-4">
            <div className="flex items-center justify-between mb-4">
                ...
                <Button size="sm" variant="flat" className="bg-emerald-500/10 text-emerald-400" startContent={<Plus size={14} />}>
                    Добавить
                </Button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {portfolio.map((item) => (
                    <Card key={item.id} className="glass-card hover:border-emerald-500/20 transition-colors">
                        <CardBody className="p-4">
                            ...
                            {item.link && (
                                <a href={item.link} target="_blank" rel="noreferrer" className="text-xs text-emerald-400 hover:text-emerald-300 flex items-center gap-1">
